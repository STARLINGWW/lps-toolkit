...
> **先读这篇**：[操作流程.md](操作流程.md) —— 硬件认识、逐节点改驱动（带截图）、
> 流程图、以及对话式向导 `python tools\lps_wizard.py` 的用法都在那里。
> 本文是刷写环节的细节手册。
...
> 所以：**一块板子刷过一次固件之后，再改角色（基站 ↔ 数据出口 ↔ 其它模式）
> 只需要改配置，不用再刷固件。** 只有新板首次使用、固件过旧（TDoA3 需 ≥ 2018.10）、
> 固件损坏时才需要刷。
>
> 判断固件能不能直接用（固件不带版本号，用能力探测代替）：
> ```powershell
> python tools\lps_config.py --port COMx --probe
> ```
> 看到 `4 - TDoA Anchor V3` 就说明可以直接用。详见 [操作流程.md](操作流程.md)。
